...
public:
    ...
    Client(int max_pending_messages, int send_ttl) : max_pending_messages(max_pending_messages), send_ttl(send_ttl) {}
    ...
    void SetOnSocketError(std::function<void(const std::string&)>&& on_socket_error) {
